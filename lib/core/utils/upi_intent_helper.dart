...
///
/// For a **bank/merchant QR** the original payload is replayed *verbatim* —
/// every field (`pa`, `pn`, `mc`, `mode`, `purpose`, `sign`, `orgid`, `tid`…)
...
///    `mode`/`purpose` downgrades it to P2P, which hits NPCI's per-payee
///    "maximum payments in 24 hours" inbound cap — fine for a resident,
///    fatal for a society VPA collecting from everyone. Verified end-to-end
///    2026-07-01: with the full merchant params replayed, payment apps show
///    the recognized merchant and settle as P2M.
///  * A `sign` is base64 (`+`, `/`, `=`). Decoding then re-encoding it — e.g.
...
///    the raw string.
///
/// A manual UPI VPA (`UPI_VPA`, a personal address with no merchant payload)
...
class UpiIntentHelper {
  ...
  /// merchant identity (mc/mode/purpose/sign…) is preserved exactly as scanned.
